"""v4 training: same protocol as v2/v3, v4 model table via build_v4."""
import json
import time
...
from models.history_set_encoder import build_v4
...
def train_one_v4(label, seed, conn, cfg, lc, data, outdir, ckptdir, epochs=24, steps=48,
                 batch=16, init=None):
    ...
    model, spec = build_v4(base_label, conn)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        row = dict(epoch=epoch, train_loss=sum(losses) / len(losses), val_loss=vl,
                   ...
                   gradient_max=max(grads), elapsed_seconds=time.monotonic() - start_time)
        history.append(row)
        blob = dict(state_dict=model.state_dict(), spec=spec, label=label, seed=seed, epoch=epoch,
                    ...
                    validation=row, protocol='latent_state_v4')
        ...
        print(f'{label} seed={seed} ep={epoch} loss={vl:.5f} F1={vm["spike_f1"]:.4f} '
              f'V={vm["v_rmse"]:.4f} sec={row["elapsed_seconds"]:.1f}', flush=True)
        if bad >= 6:
            break
